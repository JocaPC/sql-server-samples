    ...
    public class HeroesController : Controller
    {
        ...
        public async Task Get(string name)
        {
            if(string.IsNullOrEmpty(name))
                await SqlPipe.Sql("select * from Hero for json path").Stream(Response.Body, "[]");
            else
            {
        ...
        {
            await SqlPipe.Sql("select * from Hero where id = "+ id +" FOR JSON PATH, WITHOUT_ARRAY_WRAPPER").Stream(Response.Body, "{}");
        }
